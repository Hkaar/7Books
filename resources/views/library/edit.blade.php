  ...
  <div class="container flex-fill d-flex flex-column">
    <div class="row flex-fill mt-auto">
      ...
      <div class="col-12 col-md-6 d-flex align-items-center justify-content-center">
        <div class="container">
          <form method="POST" action="{{ route('libraries.update', $library->id) }}" enctype="multipart/form-data" class="shadow p-3 rounded">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
              ...
              <select name="region_id" id="region_id" class="form-control">
                ...
                @foreach ($regions as $region)
                  <option value="{{ $region->id }}" {{ $library->region_id === $region->id ? 'selected' : '' }}> {{ $region->name }} </option>
                @endforeach
              </select>
            </div>
